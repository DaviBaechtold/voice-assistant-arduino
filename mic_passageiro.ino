...
// Para Voice Assistant - Passageiro
...
// Objetos WiFi
WiFiUDP udp;
int localPort = 2391; // Porta específica do passageiro
...
void setup() {
  ...
  Serial.println("Dispositivo: PASSAGEIRO");
...
void loop() {
  ...
  if (audioReady) {
    ...
    if (millis() - lastStatus > 5000) { // A cada 5 segundos
      Serial.print("[PASSAGEIRO] Enviando áudio... Samples: ");
...
void sendAudioData() {
  ...
  packet_header.device_id = 2; // ID do PASSAGEIRO
...
void printConfig() {
  ...
  Serial.print("Device ID: 2 (PASSAGEIRO)");
  Serial.println("=====================\n");
}